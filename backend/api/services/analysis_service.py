...
class AnalysisService:
    ...
    async def start_analysis(self, run: AnalysisRun) -> None:
        ...
        from app.project_imports import set_data_source_override
        set_data_source_override(run.data_source)  # type: ignore[arg-type]
    ...
    def _execute_pipeline(self, run: AnalysisRun) -> dict[str, Any]:
        ...
        """
        rid = run.run_id
        logger.info(f"[{rid}] === PIPELINE STARTED === query='{run.query}' data_source={run.data_source}")

        # Import agent modules from project root (lazy load due to heavy deps)
        from app.project_imports import config, get_agent_mode, get_agent_modules
        agent_modules = get_agent_modules()
        setup_agent_logging = agent_modules["setup_agent_logging"]
        run_pipeline = agent_modules["run_pipeline"]
        set_shared_data = agent_modules["set_shared_data"]
        ...
        for date_dir in reports_dir.iterdir():
            ...
            for run_dir in date_dir.iterdir():
                ...
                try:
                    ...
                    # Support both old 'mode' and new 'data_source' field
                    data_source = metadata.get("data_source") or metadata.get("mode", "sample_default")
